...
- [Giới thiệu về thread](#heading-giới-thiệu-về-thread)
- [Virtual thread](#heading-virtual-thread)
    - [Đặc điểm của virtual thread](#heading-đặc-điểm-của-virtual-thread)
    - [Tạo virtual thread](#heading-tạo-virtual-thread)
    - [Virtual thread so với platform thread](#heading-virtual-thread-so-với-platform-thread)
- [Các vấn đề với thread](#heading-các-vấn-đề-với-thread)
    - [Deadlock (Bế tắc)](#heading-deadlock-bế-tắc)
    - [Starvation (Bỏ đói)](#heading-starvation-bỏ-đói)
    - [Livelock (Bế tắc sống)](#heading-livelock-bế-tắc-sống)
    - [Race condition (Điều kiện tranh đua)](#heading-race-condition-điều-kiện-tranh-đua)
- [Viết mã an toàn luồng](#heading-viết-mã-an-toàn-luồng)
    - [Truy cập dữ liệu với `volatile`](#heading-truy-cập-dữ-liệu-với-volatile)
    - [Bảo vệ dữ liệu với các class atomic](#heading-bảo-vệ-dữ-liệu-với-các-class-atomic)
    - [Khối synchronized](#heading-khối-synchronized)
    - [Đồng bộ hoá trên method](#heading-đồng-bộ-hoá-trên-method)
    - [Interface `Lock`](#heading-interface-lock)
    - [Class `CyclicBarrier`](#heading-class-cyclicbarrier)
- [Concurrency API](#heading-concurrency-api)
    - [Interface `ExecutorService`](#heading-interface-executorservice)
    - [Nộp tác vụ](#heading-nộp-tác-vụ)
    - [Interface `Callable`](#heading-interface-callable)
    - [Lập lịch tác vụ](#heading-lập-lịch-tác-vụ)
    - [Các factory method của Executors](#heading-các-factory-method-của-executors)
    - [Executor nhận biết virtual thread](#heading-executor-nhận-biết-virtual-thread)
- [Concurrent collection](#heading-concurrent-collection)
- [Parallel stream](#heading-parallel-stream)
    - [Tạo parallel stream](#heading-tạo-parallel-stream)
    - [Phân rã song song (parallel decomposition)](#heading-phân-rã-song-song-parallel-decomposition)
    - [Những method của Stream thực hiện tác vụ dựa trên thứ tự](#heading-những-method-của-stream-thực-hiện-tác-vụ-dựa-trên-thứ-tự)
    - [Rút gọn parallel stream](#heading-rút-gọn-parallel-stream)
    - [Kết hợp kết quả trong parallel stream](#heading-kết-hợp-kết-quả-trong-parallel-stream)
...
**1. Dòng mã nào sau đây tạo và khởi chạy đúng một virtual thread mới?**
...
**2. Phương án nào dùng đúng khối `synchronized` để đảm bảo chỉ một thread tại một thời điểm chạy được vùng then chốt tăng một bộ đếm dùng chung?**
...
**3. Phát biểu nào sau đây về các class atomic là đúng? (Chọn tất cả đáp án đúng)**

**A)** `AtomicInteger` thuộc package `java.util.concurrent.atomic`, nhưng nó không cung cấp thao tác nguyên tử để tăng và giảm.  
**B)** `AtomicReference` chỉ dùng được với kiểu tham chiếu, không dùng được với kiểu nguyên thuỷ.  
**C)** `AtomicLong` hỗ trợ thao tác nguyên tử trên giá trị `long`, gồm cả method `getAndIncrement()` và `compareAndSet()`.  
**D)** `AtomicBoolean` dùng được để thực hiện thao tác số học nguyên tử trên giá trị `boolean`.


**4. Đoạn mã nào sau đây dùng đúng interface `Lock` để đảm bảo truy cập an toàn luồng vào tài nguyên dùng chung?**
...
**5. Đoạn mã nào sau đây minh hoạ đúng cách dùng `ExecutorService` với khối `try-with-resources`?**
...
**6. Đoạn mã nào sau đây minh hoạ đúng cách lấy kết quả từ một tác vụ `Callable` bằng `ExecutorService` với `try-with-resources`?**
...
**A)** `ConcurrentHashMap` cho phép thao tác đọc và ghi đồng thời, và thao tác lấy dữ liệu không bị chặn ngay cả khi đang có cập nhật.  
**B)** `CopyOnWriteArrayList` được tối ưu cho những tình huống có số thao tác ghi nhiều hơn hẳn số thao tác đọc.  
**C)** `ConcurrentSkipListSet` không giữ phần tử theo thứ tự sắp xếp.  
**D)** Những cài đặt `BlockingQueue` như `LinkedBlockingQueue` cho phép thêm và xoá phần tử đồng thời mà không cần bất kỳ cơ chế khoá nội bộ nào.


**8. Phát biểu nào sau đây về parallel stream là đúng?**

**A)** Parallel stream luôn cải thiện hiệu năng chương trình nhờ tận dụng nhiều thread.  
**B)** Parallel stream có thể dẫn tới kết quả sai nếu những thao tác được thực hiện không an toàn luồng.  
**C)** Thứ tự phần tử trong parallel stream luôn được giữ nguyên so với stream gốc.  
**D)** Dùng parallel stream đảm bảo các thao tác trên phần tử sẽ chạy theo một thứ tự cố định.


**9. Đoạn mã nào sau đây minh hoạ đúng cách rút gọn một parallel stream để tính tổng các phần tử?**
