...
- [Mảng (Array)](#heading-mảng-array)
    - [Tạo và khởi tạo mảng](#heading-tạo-và-khởi-tạo-mảng)
    - [Mảng vô danh (anonymous array)](#heading-mảng-vô-danh-anonymous-array)
    - [Dùng mảng](#heading-dùng-mảng)
    - [Mảng nhiều chiều](#heading-mảng-nhiều-chiều)
    - [Class `java.util.Arrays`](#heading-class-javautilarrays)
    - [Dùng `fill()`](#heading-dùng-fill)
- [Generics](#heading-generics)
    - [Hiểu về type erasure](#heading-hiểu-về-type-erasure)
    - [Tạo class generic](#heading-tạo-class-generic)
    - [Quy ước đặt tên cho generics](#heading-quy-ước-đặt-tên-cho-generics)
    - [Viết method và constructor generic](#heading-viết-method-và-constructor-generic)
    - [Trả về kiểu generic](#heading-trả-về-kiểu-generic)
    - [Nạp chồng method generic](#heading-nạp-chồng-method-generic)
    - [Cài đặt interface generic](#heading-cài-đặt-interface-generic)
    - [Tạo record generic](#heading-tạo-record-generic)
    - [Giới hạn kiểu generic](#heading-giới-hạn-kiểu-generic)
- [Collections Framework](#heading-collections-framework)
- [Interface `List`](#heading-interface-list)
    - [Tạo một `List`](#heading-tạo-một-list)
    - [Làm việc với method của `List`](#heading-làm-việc-với-method-của-list)
- [Interface `Set`](#heading-interface-set)
    - [Tạo một `Set`](#heading-tạo-một-set)
    - [Làm việc với method của `Set`](#heading-làm-việc-với-method-của-set)
- [Interface `Deque`](#heading-interface-deque)
    - [Tạo một `Deque`](#heading-tạo-một-deque)
    - [Làm việc với method của `Deque`](#heading-làm-việc-với-method-của-deque)
- [Interface `Map`](#heading-interface-map)
    - [Tạo một `Map`](#heading-tạo-một-map)
    - [Làm việc với method của `Map`](#heading-làm-việc-với-method-của-map)
    - [Override `hashCode()`](#heading-override-hashcode)
- [Sắp xếp dữ liệu](#heading-sắp-xếp-dữ-liệu)
    - [Interface `Comparable`](#heading-interface-comparable)
    - [Interface `Comparator`](#heading-interface-comparator)
    - [So sánh `Comparable` và `Comparator`](#heading-so-sánh-comparable-và-comparator)
    - [`Collections.sort` và `Collections.binarySearch`](#heading-collectionssort-và-collectionsbinarysearch)
- [Tóm tắt các loại collection](#heading-tóm-tắt-các-loại-collection)
    - [Bảng 1: Interface collection và các cài đặt](#heading-bảng-1-interface-collection-và-các-cài-đặt)
    - [Bảng 2: Chức năng của các interface collection cốt lõi](#heading-bảng-2-chức-năng-của-các-interface-collection-cốt-lõi)
    - [Bảng 2.1: Chức năng của interface Map](#heading-bảng-21-chức-năng-của-interface-map)
    - [Bảng 3: Method chung của collection](#heading-bảng-3-method-chung-của-collection)
    - [Bảng 4: Method riêng của List](#heading-bảng-4-method-riêng-của-list)
    - [Bảng 5: Method riêng của Set](#heading-bảng-5-method-riêng-của-set)
    - [Bảng 6: Method riêng của Deque](#heading-bảng-6-method-riêng-của-deque)
    - [Bảng 7: Method riêng của Map](#heading-bảng-7-method-riêng-của-map)
...
**2. Định nghĩa method generic nào sau đây khai báo đúng một method trả về phần tử đầu tiên của mảng cho trước?**
...
**3. Kết quả của việc biên dịch và chạy đoạn mã sau là gì?**
...
**B)** Mã biên dịch được và in ra:
   ```
   1 2 3
   1.1 2.2 3.3
   ```

**C)** Mã không biên dịch được do lỗi trong method `printList`.  
**D)** Mã không biên dịch được do lỗi trong method `main`.  
**E)** Mã biên dịch được nhưng ném exception lúc chạy.


**4. Chương trình sau in ra gì?**
...
**5. Những phát biểu nào sau đây về interface `Set` là đúng? (Chọn tất cả đáp án đúng.)**

**A)** `Set` cho phép phần tử trùng lặp.  
**B)** Phần tử trong `Set` được giữ theo thứ tự chèn vào.  
**C)** Interface `Set` có các method để thêm, xoá và kiểm tra sự tồn tại của phần tử.  
**D)** Interface `Set` được cài đặt bởi những class như `HashSet`, `LinkedHashSet` và `TreeSet`.  
**E)** `Set` đảm bảo hiệu năng hằng số cho các thao tác cơ bản (add, remove, contains).
...
**9. Chương trình sau in ra gì khi dùng `Comparator` được cung cấp?**
