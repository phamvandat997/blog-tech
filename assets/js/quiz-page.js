...
function openQuizList() {
  ...
  const docsWithQuiz = (window.DOCUMENTS || []).filter((d) => (d.questions || 0) > 0);
  ...
  const sectionTabs = [
    { id: \"all\", name: \"Tất cả\" },
    ...(window.SECTIONS || []).filter((s) => docsWithQuiz.some((d) => d.section === s.id))
  ];
  ...
  function renderCards() {
    ...
    listEl.innerHTML = filtered.map((doc) => {
      const sec = (window.SECTIONS || []).find((s) => s.id === doc.section);
